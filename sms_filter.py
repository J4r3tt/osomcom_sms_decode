...
def handle_message(**kargs):
    ...
    while True:
        ...
        if data[0:2] == '\x02\x04': #GSM_TAP header Version02 & HeaderLength 16bytes
            ...
            if (address_field >> 2) & 0x1F == 3: # GSM SMS && frame type == information frame
                            ...
                            try:
                                ...
                                if is_uplink:
                                    ...
                                    if sms_submit & 0x03 == 0x01:
                                        ...
                                        print "Time:%s\nFrom: %s\nTo:%s" % (GetCurrentTime(),from_number,to_number)
                                ...
                                if is_sms_deliver:
                                    try:
                                        ...
                                        if not is_mms:
                                            print 'Message:' + sms.decode('UTF-16BE')
                                            print '-'*74
                                        else:
                                            print "Time:%s\n MMS message." % GetCurrentTime()
                                            print '-'*74

                                    except:
                                        print "Time:%s\nCan't Decode The Message" % GetCurrentTime()
                                        print '-'*74
                                    ...
                                    print "Time:%s\nSMS Status Report. " % GetCurrentTime()
                                    print '-'*74
                            except:
                                print "Time:%s\nUnexpected packets format." % GetCurrentTime()
                                print '-'*74
