                        ...
                        if rp.RP_message_type == 1:
                            ...
                            if tpdu.TP_mti == 0x00:
                                ...
                                if tpdu.TP_charaterset == 2:
                                    try:
                                        center_time=time.strptime(tpdu.time_stamp,"%y%m%d%H%M%S")
                                    except:
                                        center_time=time.localtime()
                                    #hexdump.hexdump(tpdu.data)
                                    print("%s (Downlink) [SMS from %s] %s" % (
                                        time.strftime("%Y-%m-%d %H:%M:%S", center_time),tpdu.TP_origin, tpdu.data.decode("utf-16be").encode("utf-8")))
                                    # except:
                                    #     print(
                                    #         "(Downlink) Decode Failed!Maybe wrong format")
                                # it's mmse
                                    ...
                                    try:
                                        ...
                                        center_time=time.localtime()
                                    print("%s (Downlink) [MMSE from %s] " % (time.strftime("%Y-%m-%d %H:%M:%S", center_time),tpdu.TP_origin))
                            # SMS-STATUS REPORT
                            if tpdu.TP_mti == 0x02:
                                try:
                                        center_time=time.strptime(tpdu.time_stamp,"%y%m%d%H%M%S")
                                        ...
                                        center_time=time.localtime()
                                print(
                                    "%s (Downlink) [SMS status report from %s] " % (time.strftime("%Y-%m-%d %H:%M:%S", center_time),tpdu.TP_origin))
                            ...
                            if tpdu.TP_mti == 0x01:
                                if tpdu.TP_charaterset == 2:
                                    ...
                                    print("%s (Uplink)[SMS to %s] %s" % (
                                            time.strftime("%Y-%m-%d %H:%M:%S", center_time),tpdu.TP_dest, tpdu.data.decode("utf-16be").encode("utf-8")))
                                else:
                                    center_time=time.localtime()
                                    print("%s (Uplink) MMSE uplink" % time.strftime("%Y-%m-%d %H:%M:%S", center_time))
                        # RP-ACK
                        elif rp.RP_message_type == 2:
                            # SMS-DELIVER REPORT
                            print "-"*90
                            ceter_time=time.localtime()
                            print("%s (Uplink) SMS status report" % time.strftime("%Y-%m-%d %H:%M:%S", center_time))
